...
// bir egzersizde yapılan değerin (v), kaydedilmiş max teste (Rekor) göre göreli yoğunluğu — max test yoksa 1 (mevcut davranış değişmez)
export function relIntensity(e,v){const lm=lastMax(e.id);if(!lm||!(lm.v>0))return 1;return Math.max(0,Math.min(1,v/lm.v));}
...
export function sysRecovery(s){const now=Date.now(),R=S.cfg,cap=Math.max(R.lookbackHours,30*24),groups={};
  for(let i=S.logs.length-1;i>=0;i--){const l=S.logs[i];const ageH=(now-l.t)/3600000;if(ageH>cap)break;
    ...
    const key=l.e+'|'+dayKey(dt(l));const g=groups[key]||(groups[key]={t:0,r:0,e,w,n:0,vsum:0});if(l.t>g.t)g.t=l.t;if(l.r>g.r)g.r=l.r;g.n++;g.vsum+=(l.v||0);}
  let sum=0;for(const k in groups){const g=groups[k];let f=rpeFactor(g.r);
    ...
    f*=relIntensity(g.e,g.n?g.vsum/g.n:0);
